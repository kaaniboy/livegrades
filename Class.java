...
public class Class {
	...
	public Class(String name, String room, String teacher, double grade, String classPageHTML) {
		this.name = name;
		this.room = room;
		...
		for (Element e : assignmentsList.select(".altrow1,.altrow2")) {
			...
			Assignment assignment = new Assignment(date, assignmentName, type, score,
					points);
	...
	private void loadWeights(Element weightsTable) {
		for(Element e: weightsTable.select(".altrow1,.altrow2")) {
			...
			double receivedPoints = Double.parseDouble(e.select("td").get(2).html().toString());
			double totalPoints = Double.parseDouble(e.select("td").get(3).html().toString());
			Double parsedPercentage = .01 * Double.parseDouble(categoryPercentage.replace("%", ""));
			
			WeightCategory weight = new WeightCategory(categoryTitle, parsedPercentage, receivedPoints, totalPoints);
			weights.add(weight);
		}
	}
